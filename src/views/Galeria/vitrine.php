...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-3xl font-semibold tracking-tight text-slate-900">Vitrine</h1>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-5">
        <div class="grid grid-cols-1 gap-3 lg:grid-cols-3">
            ...
            <div class="lg:col-span-2 grid grid-cols-1 gap-3 sm:grid-cols-2">
                <div>
                    <label for="filterType" class="text-sm font-medium text-slate-700">Tipo</label>
                    <select id="filterType" class="mt-1 h-10 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 focus:border-slate-400 focus:outline-none focus:ring-4 focus:ring-slate-100">
   ...
   <div id="vitrineTableBody">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            ...
            <div class="js-vitrine-card overflow-hidden rounded-2xl border border-slate-200 bg-white transition hover:shadow-xl"
                data-nome="<?= strtolower(htmlspecialchars($item->getTitulo())) ?>"
                data-type="<?= htmlspecialchars($item->getTipo() ?? '') ?>"
...
<script type="module" src="<?= Url::path('/js/vitrine.js') ?>"></script>